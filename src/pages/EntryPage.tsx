...
interface SortableHeaderBlockItemProps {
  key?: string | number;
  block: HeaderBlock;
  onUpdate: (u: Partial<HeaderBlock>) => void;
  onRemove: () => void;
  isFullScreen: boolean;
  onExpand: () => void;
}

function SortableHeaderBlockItem({ block, onUpdate, onRemove, isFullScreen, onExpand }: SortableHeaderBlockItemProps) {
  ...
  const saveToParent = (contents: Record<string, string>) => {
    if (activeVariationId === 'default' && variations.length === 1) {
      onUpdate({ content: contents['default'] });
    } else {
      const newVariations = variations.map(v => 
  ...
  return (
    ...
    >
      <div 
        {...attributes} 
        {...listeners}
        className="p-4 flex flex-col cursor-grab active:cursor-grabbing hover:bg-bg-surface-hover/50 transition-colors shrink-0 touch-none"
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-y-auto pb-24 relative">
            ...
            >
              {headerBlocks.map((block) => (
                <SortableHeaderBlockItem 
                  ...
                  onExpand={() => setFullScreenBlockId(fullScreenBlockId === block.id ? null : block.id)}
                />
              ))}
            </SortableContext>
          </DndContext>
      ...
      <button 
        ...
        className="fixed bottom-6 right-6 w-14 h-14 bg-bg-surface-hover border border-border-main text-text-main rounded-2xl shadow-2xl flex items-center justify-center hover:bg-bg-surface-hover transition-all z-10"
      >
      ...
      {showTagModal && (
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-end sm:items-center justify-center p-4">
          <div className="bg-bg-surface rounded-t-3xl sm:rounded-3xl p-6 w-full max-w-md border border-border-main shadow-2xl max-h-[85vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto mb-6 pr-2">
              <div className="text-sm text-text-muted mb-3">Click once to select as sub-tag, double click to set as main tag.</div>
              <div className="flex flex-wrap gap-2">
                {tags.map(tag => {
            ...
            <div className="shrink-0 pt-4 border-t border-border-main space-y-4">
              ...
              <div className="flex gap-2">
                <input 
                  ...
                  className="flex-1 bg-bg-main border border-border-main rounded-xl px-4 py-2 text-text-main focus:outline-none focus:ring-2 focus:ring-accent"
                  onKeyDown={e => e.key === 'Enter' && handleCreateCustomTag()}
                />
                <button 
                  onClick={handleCreateCustomTag}
                  className="px-4 py-2 bg-text-main text-bg-main rounded-xl font-medium hover:bg-text-secondary transition-colors"
                >
                  Add
                </button>
              </div>
