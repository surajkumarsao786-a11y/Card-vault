...
function SortableBlock({ block, isExpanded, onToggle, onUpdate, onDelete, onCopy, copiedId, onExpand, isFullScreen }: SortableBlockProps) {
  ...
  return (
    ...
    >
      <div 
        {...attributes} 
        {...listeners}
        className="p-4 flex flex-col cursor-pointer hover:bg-bg-surface-hover/50 transition-colors shrink-0 touch-none"
        onClick={onToggle}
      >
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 shrink-0 ml-4">
            ...
            <button
              ...
              title="Copy content"
            >
              {copiedId === block.id ? <Check className="w-5 h-5 text-green-400" /> : <Copy className="w-5 h-5" />}
            </button>
            <button
...
export default function PromptDetail() {
  ...
  const handleUpdateBlock = (id: string, updates: Partial<HeaderBlock>) => {
  ...
  const copyToClipboard = async (e: React.MouseEvent, block: HeaderBlock) => {
    e.stopPropagation();
    try {
      const variations = block.variations || [{ id: 'default', name: 'V1', content: block.content }];
      const activeVariationId = block.activeVariationId || variations[0].id;
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-bg-main relative">
      ...
      <div className="flex-1 overflow-y-auto p-4 flex">
                ...
                <div className="space-y-4">
                  {blocks.map((block) => (
                    <SortableBlock
                      ...
                      onCopy={(e) => copyToClipboard(e, block)}
                      copiedId={copiedId}
        ...
        {/* History Sidebar */}
        {showHistory && (
          <div className="fixed top-16 right-0 bottom-0 w-80 bg-bg-surface border-l border-border-main p-4 overflow-y-auto z-30 shadow-2xl">
